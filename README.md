...
| `test_http_server.py` | Functional test of `http_server.py`'s rotation, month-name exemption and rate limit: runs the real server on a loopback port against a temp dir, no mocks. `python3 test_http_server.py`, exits nonzero on failure. Every case checks CONSERVATION OF ROWS -- a scheme that overwrites its rotated file passes any does-it-rotate check while destroying data. |
