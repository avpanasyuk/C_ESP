...
namespace avp {
  ...
  protected:
    ...
    void GetIP() {
      if(!WiFi.hostByName(server, remote_addr)) Error = "Can not resolve name";
    }
  ...
  public:
    explicit Client(const char *server, uint16_t port = 80, unsigned long Timeout_ms = Client_::DEFAULT_TIMEOUT_MS)
