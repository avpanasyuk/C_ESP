...
| `http_server.py` | Companion server: writes each POSTed `<filename>,<csv>` body to `<log-dir>/<filename>` with a timestamp column prepended, **and** serves firmware at `/firmware/<name>.bin` for `FleetServerOTA.hpp` (MD5-gated: 304 when the device already runs the image). Once a log passes `--max-log-bytes` (default 10 MiB) it is renamed aside as `<file>.<YYYYmmdd-HHMMSS>` and a fresh one starts — **nothing is ever deleted**, so a series is read as `<file>` plus its timestamped siblings (they sort chronologically as plain strings). What actually stops a chatty device filling the disk is `--max-rows-per-min` (default 120, per filename): excess rows get a 429 and one alert to `--alert-email`, which a device on a sane cadence can never trigger. `--dir-quota-bytes` alerts if the whole log dir passes a threshold, and also never deletes. Also records fleet-OTA pulls/confirms and refuses to serve a blacklisted MD5 (see `fleet_ota.py`). |
