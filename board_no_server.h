...
struct ESP_board_no_server {
  enum ConnectionStatus_t { IDLE, TRYING_TO_CONNECT, AP_MODE, CONNECTED };
  ...
  static Options_t Default() {
    return {"", "L", "group224", [](enum ConnectionStatus_t) {}};
  } // Default
...
public:
  ...
  ESP_board_no_server(
    ...
    if (WiFi.isConnected()) post_connection();
      ...
      if (WiFi.isConnected()) post_connection();
      else { // if still not connected switching to AP mode. In this mode we
             // got to connect to ESP generated WiFi
        WiFi.mode(WIFI_AP);
        WiFi.softAP(Name, "");
        ip = WiFi.softAPIP();
