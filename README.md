...
| `http_server.py` | Companion server: writes each POSTed `<filename>,<csv>` body to `<log-dir>/<filename>` with a timestamp column prepended, **and** serves firmware at `/firmware/<name>.bin` for `FleetServerOTA.hpp` (MD5-gated: 304 when the device already runs the image). **Size rotation is off by default** (`--max-log-bytes 0`) — a device that puts the month in its filename already gets one bounded file per month, and splitting that costs every reader a chance to see only part of a month. What stops a chatty device filling the disk is `--max-rows-per-min` (default 120, per filename): excess rows get a 429 and one alert to `--alert-email`, a rate a device on a sane cadence can never reach. `--dir-quota-bytes` alerts if the whole log dir passes a threshold. Neither ever deletes. If you do set `--max-log-bytes`, the over-size file is renamed aside as `<file>.<YYYYmmdd-HHMMSS>` and kept — a series is then `<file>` plus its timestamped siblings, which sort chronologically as plain strings. Also records fleet-OTA pulls/confirms and refuses to serve a blacklisted MD5 (see `fleet_ota.py`). |
